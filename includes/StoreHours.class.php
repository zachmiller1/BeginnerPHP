...
class StoreHours {
  ...
  function __construct($hours = array(), $exceptions = array(), $template = array()) {
    ...
    if(!isset($this->template['open'])) {
      $this->template['open'] = "<strong class='open'>Yes, we're open! Today's hours are {%hours%}.</strong>";
    }
    if(!isset($this->template['closed'])) {
      $this->template['closed'] = "<strong class='closed'>Sorry, we're closed. Today's hours are {%hours%}.</strong>";
    }
    if(!isset($this->template['closed_all_day'])) {
      $this->template['closed_all_day'] = "<strong class='closed'>Sorry, we're closed.</strong>";
    }
